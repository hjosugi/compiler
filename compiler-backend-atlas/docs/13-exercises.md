...
既存の`// comment`の次に`/* ... */` commentを実装し、identifierをASCII限定にするかUnicode identifierを許すかを仕様へ明記します。
...
`bench/runner.py`へdirect backendを追加し、LLVM O0と同一process/cold processを分けて測ります。
