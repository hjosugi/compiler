...
class FunctionLowerer:
    ...
    def emit(
        ...
        self.current.instructions.append(
            Instruction(op, result, type_name, args, attrs or {})
        )
        return result
    ...
    def lower(self) -> KIRFunction:
        ...
        return KIRFunction(
            self.function.name, params, self.function.return_type, self.blocks
        )
    ...
    def lower_short_circuit(self, expr: ast.BinaryExpr) -> str:
        ...
        result = self.emit(
            ...
            "Bool",
            attrs={
                "incoming": [(branch_block.label, short_value), (rhs_end, rhs_value)]
            },
        )
        assert result is not None
        return result
