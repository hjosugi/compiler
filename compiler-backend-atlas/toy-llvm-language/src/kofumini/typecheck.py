...
def check(program: ast.Program) -> TypeInfo:
    ...
    def type_expr(expr: ast.Expr, env: dict[str, str], function_name: str) -> str:
        if isinstance(expr, ast.IntLiteral):
            ...
            if condition != "Bool":
                raise TypeCheckError(
                    f"type error in {function_name}: if condition must be Bool"
                )
    ...
    for function in program.functions:
        if not function.body or not isinstance(function.body[-1], ast.ReturnStmt):
            raise TypeCheckError(
                f"type error in {function.name}: function must end with return"
            )
        ...
        for statement in function.body:
            if returned:
                raise TypeCheckError(
                    f"type error in {function.name}: statement after return"
                )
            if isinstance(statement, ast.LetStmt):
                ...
                if actual != "Int":
                    raise TypeCheckError(
                        f"type error in {function.name}: print accepts Int only"
                    )
