...
Stage6  → typed KofuMiniへ接続                    (SSA KIR / LLVM IR / native)
```
...
diff tutorial/stage2_interp.py tutorial/stage3_functions.py
```
...
(b) lexical slotをlive rangeが重ならない範囲で再利用する = レジスタ割付の前身
(c) 定数畳み込み: CONST a; CONST b; BINOP + を CONST (a+b) に潰すピープホール最適化

---

## Stage 6 — typed SSAとLLVMへ接続する

Stage 1–5のMiniは、差分を小さく保つため型注釈のない文中心のdialectです。次に別dialectのKofuMiniを使い、型付きAST、SSA KIR、LLVM IRという新しい表現を一周します。syntaxとASTは同一ではありませんが、lexer、parser、type checker、IR、backendという責務は連続しています。

| 実行経路 | 場所 | 実行形態 |
|---|---|---|
| Mini tree walk | `tutorial/stage3_functions.py` | ASTをその場で歩く |
| Mini bytecode VM | `tutorial/stage5_bytecode.py` | stack命令 + 仮想machine |
| KofuMini reference | `src/kofumini/interpreter.py` | typed SSA KIRを直接評価 |
| KofuMini LLVM/native | `src/kofumini/llvm_emitter.py` | textual LLVM IRをClangへ渡す |
...
4. 本repositoryの`docs/01-compiler-pipeline.md` → `05-ir-ssa-optimization.md` → `03-llvm-backend.md`
