...
EXAMPLES = ROOT / "examples" / "kofumini"
...
class CompilerPipelineTests(unittest.TestCase):
    ...
    def test_source_filename_is_escaped_for_llvm_ir(self) -> None:
        compilation = compile_source("fn main() -> Int { return 0; }", 'quote"\\日本語.kofu')
        source_line = compilation.llvm_ir.splitlines()[1]
...
@unittest.skipUnless(
    shutil.which("clang") and os.environ.get("SKIP_LLVM") != "1",
    "native LLVM test is disabled or clang is not installed",
)
